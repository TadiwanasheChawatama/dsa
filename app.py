...
from sortingalgorithms import bubble_sort, selection_sort, insertion_sort, merge_sort, quick_sort,heap_sort, quick_runtime, merge_runtime
...
def index():
    ...
    if request.method == "POST":
        ...
        accommodation = request.form.get('accommodation') == 'True' 
...
def sort_data():
    ...
    if sort_by and sort_order and sort_algorithm:  
        ...
        if sort_algorithm == 'bubble_sort':
            all_students, elapsed = bubble_sort(all_students, sort_by)
            update_algorithms('bubble sort', elapsed, 'sort')
        elif sort_algorithm == 'selection_sort':
            all_students, elapsed = selection_sort(all_students, sort_by)
            update_algorithms('selection sort', elapsed, 'sort')
        elif sort_algorithm == 'insertion_sort':
            all_students, elapsed = insertion_sort(all_students, sort_by)
            update_algorithms('insertion sort', elapsed, 'sort')
        elif sort_algorithm == 'quick_sort':
            all_students, elapsed = quick_sort(all_students, sort_by), quick_runtime
            update_algorithms('quick sort', elapsed, 'sort')
        elif sort_algorithm == 'merge_sort':
            all_students, elapsed = merge_sort(all_students, sort_by), merge_runtime
            update_algorithms('merge sort', elapsed, 'sort')
        elif sort_algorithm == 'heap_sort':
            all_students, elapsed = heap_sort(all_students, sort_by)
            update_algorithms('heap sort', elapsed, 'sort')
        else:
...
def search_data():
    ...
    if search_algorithm == 'binary_search':
        matching_students, elapsed = binary_search(all_students, search_term)
        update_algorithms('binary', elapsed, 'search')
    elif search_algorithm == 'linear_search':
        matching_students, elapsed = linear_search(all_students, search_term)
        update_algorithms('linear', elapsed, 'search')
    elif search_algorithm == 'hash_table_search':
        matching_students, elapsed = hash_table_search(all_students, search_term)
        update_algorithms('hash table', elapsed, 'search')
    elif search_algorithm == 'interpolation_search':
        matching_students, elapsed = interpolation_search(all_students, search_term)
        update_algorithms('interpolation', elapsed, 'search')
    elif search_algorithm == 'ternary_search':
        matching_students, elapsed = ternary_search(all_students, search_term)
        update_algorithms('ternary', elapsed, 'search')
    elif search_algorithm == 'exponential_search':
        matching_students, elapsed = exponential_search(all_students, search_term)
        update_algorithms('exponential', elapsed, 'search')
...
algorithms = {}
algorithmsSearch = {}
def update_algorithms(algorithm, runtime, type):
    if type == 'sort':
        algorithms[algorithm] = runtime
        plot_runtime(algorithms, type)
    else:
        algorithmsSearch[algorithm] = runtime
        plot_runtime(algorithmsSearch, type)

def plot_runtime(algorithms, type):
    if type == 'sort':
        # Plotting
        plt.figure(figsize=(10, 6))
        plt.plot(algorithms.keys(), algorithms.values(), color='#4c72b0', marker='o')
        plt.xlabel('Sort Algorithm', fontweight='bold', fontsize=12)
        plt.ylabel('Runtime (s)', fontweight='bold', fontsize=12)
        plt.xticks(rotation=30, ha='right')
        plt.tight_layout()
        plt.title('Algorithms  runtime')
        runtime_plot_path = os.path.join(app.config['STATIC_FOLDER'], 'images', 'runtime.png')
        plt.savefig(runtime_plot_path)
        plt.close()
        return runtime_plot_path
    else:
        plt.figure(figsize=(10, 6))
        plt.plot(algorithms.keys(), algorithms.values(), color='#4c72b0', marker='o')
        plt.xlabel('Search Algorithm', fontweight='bold', fontsize=12)
        plt.ylabel('Runtime (s)', fontweight='bold', fontsize=12)
        plt.xticks(rotation=30, ha='right')
        plt.tight_layout()
        plt.title('Algorithms  runtime')
        runtime_plot_path = os.path.join(app.config['STATIC_FOLDER'], 'images', 'runtime_search.png')
        plt.savefig(runtime_plot_path)
        plt.close()
        return runtime_plot_path
