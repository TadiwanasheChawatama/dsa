...
def delete_data():
    ...
    all_students = [{**student, "_id": str(student["_id"])} if isinstance(student, dict) else student for student in all_students]
...
def update_data():   
    if request.method == "POST":
        update_id = request.form.get('_id')
        firstname = request.form.get('firstname')
        lastname = request.form.get('lastname')
        date_of_birth = request.form.get('date_of_birth')
        gender = request.form.get('gender')
    ...
    students.update_one({"_id": ObjectId(update_id)}, {update_data})
